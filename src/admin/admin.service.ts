...
export class AdminService {
  ...
  /**
   * Anyone without real, current Directory access worth pitching — never
   * subscribed, subscribed and canceled, or currently past due. Excludes
   * staff accounts (nothing to pitch them) and anyone already comped via
   * adminGrantedDirectoryAccess (already converted, in the sense that
   * matters here — no subscription needed, so re-pitching them is noise).
   * Deliberately broader than "never subscribed" alone: a churned or
   * lapsed customer is exactly the kind of person a pitch email is for,
   * not just brand-new signups. Oldest signup first: the longest someone
   * has sat without paid access is the strongest signal they're worth
   * reaching out to.
   */
  async listProspects(query: ListProspectsQueryDto): Promise<Page<unknown>> {
    ...
    const now = Date.now();
    const enriched = data.map(({ subscriptions, ...user }) => {
      ...
      return {
        ...
        canceled: mostRecent?.status === 'CANCELED',
        overdue: mostRecent?.status === 'PAST_DUE',
        paid: subscriptions.some((s) => s.invoices.length > 0),
      };
    });
  ...
  // Bulk CSV/paste uploads are handled by ProspectCampaignService
  // (queues into ProspectInviteQueue for the daily-capped drip, rather
  // than sending immediately) — see AdminController.queueProspectsBulk.
